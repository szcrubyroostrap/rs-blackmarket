RSpec.describe Cart, type: :model do
  ...
  describe 'associations' do
    it { expect(subject).to have_many(:cart_products).dependent(:destroy) }
    it { expect(subject).to have_many(:products).through(:cart_products) }
    it { expect(subject).to belong_to(:user) }
  end

  describe 'validations' do
    it { is_expected.to validate_presence_of(:total_items) }
    it { is_expected.to validate_presence_of(:total_price) }
    it { is_expected.to validate_numericality_of(:total_items).is_greater_than_or_equal_to(0) }
    it { is_expected.to validate_numericality_of(:total_price).is_greater_than_or_equal_to(0) }
    it { is_expected.to define_enum_for(:status).with_values(in_process: 0, completed: 1) }
  end
